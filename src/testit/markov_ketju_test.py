...
class TestMarkovKetju(unittest.TestCase):
    def setUp(self):
        self.korpus = [['Sateen', 'jälkeen', 'maa', 'tuoksuu', 'raikkaalta'], [
    ...
    def test_generoi_tason_2_trie(self):
        ...
        self.assertTrue(
            ketju.trie.aloitussolmu.lapset['ilma'].lapset['on'].lapset['yleensä'].on_lehtisolmu)
        self.assertTrue(
            ketju.trie.aloitussolmu.lapset['maa'].lapset['tuoksuu'].lapset['raikkaalta'].on_lehtisolmu)

        self.assertEqual(
            ketju.trie.aloitussolmu.lapset['ilma'].lapset['on'].lapset['yleensä'].frekvenssi, 1)
        self.assertEqual(
            ketju.trie.aloitussolmu.lapset['maa'].lapset['tuoksuu'].lapset['raikkaalta'].frekvenssi, 1)
    ...
    def test_generoi_tason_3_trie(self):
        ...
        self.assertTrue(
            ketju.trie.aloitussolmu.lapset['ilma'].lapset['on'].lapset['yleensä'].lapset['raikas'].on_lehtisolmu)
        self.assertTrue(
            ketju.trie.aloitussolmu.lapset['jälkeen'].lapset['maa'].lapset['tuoksuu'].lapset['raikkaalta'].on_lehtisolmu)
